...
def get_ptb_data():
    ...
    if not os.path.exists('Datasets/trainDevTestTrees_PTB/trees'):
        ...
        exit()
    elif not os.path.exists('Datasets/trainDevTestTrees_PTB/trees/train.txt'):
        print("train.txt is not in ../large_files/trees/train.txt")
        print("Please download the data from http://nlp.stanford.edu/sentiment/")
        exit()
    elif not os.path.exists('Datasets/trainDevTestTrees_PTB/trees/test.txt'):
    ...
    # train set first
    for line in open('Datasets/trainDevTestTrees_PTB/trees/train.txt', encoding='utf-8'):
        line = line.rstrip()
        if line:
    ...
    # test set
    for line in open('Datasets/trainDevTestTrees_PTB/trees/test.txt', encoding='utf-8'):
        line = line.rstrip()
        if line:
            t = str2tree(line, word2idx)
            test.append(t)
